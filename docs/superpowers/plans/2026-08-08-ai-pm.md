...
- Produces:
  ...
  - `GATE_KEYWORDS: List[str]`：`["学历", "本科", "硕士"]`。
...
```
Expected: `deepseek-ai-pm` 输出 `unmapped: 0`；`deepseek-agent-harness` 输出 `unmapped: 3`（加分项纯经验描述「AI行业从业经验、AI相关产品经验」「作为小团队中的产品经理主导产品路线的经验」「其它超乎常人的与此工作相关的才能」，属预期）。职责与核心要求行必须全部映射；mission 品牌文案不参与 unmapped 统计。
...
from scripts_import import import_sync_file  # 实际入口在 scripts/sync_import.py 内，见 Step 3
```

同步导入逻辑需要可测试入口，因此放在 `src/ai_pm/sync.py`：
